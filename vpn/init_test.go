...
type testService struct {
	...
	vpnConfig vpn.ServiceConfig
}

func startTestService() *testService {
	...
	svc.httpTestSvr = httptest.NewUnstartedServer(http.HandlerFunc(
		func(w http.ResponseWriter, r *http.Request) {
			...
			Expect(passwd).To(Equal("root_p@ssw0rd"))
			if r.RequestURI != svc.httpTestSvrExpectedURI {
				http.NotFound(w, r)
				return
			}
			_, svc.httpTestSvrErr = w.Write([]byte(wireguardConfig))
		},
	))
	svc.httpTestSvr.TLS = tlsCfg
	svc.httpTestSvr.StartTLS()
	...
	err = tgts.UnmarshalJSON(
		[]byte(
			fmt.Sprintf(targets, strings.Split(svc.httpTestSvr.URL, ":")[2]),
		),
	)
	svc.target = tgts.GetTarget("fakeRecipe/fakeIAAS/")
	Expect(err).NotTo(HaveOccurred())	
...
func (svc *testService) URL() string {
	return svc.httpTestSvr.URL
}

func (svc *testService) stop() {
	svc.httpTestSvr.Close()
}

func (svc *testService) parseConnectResponse(vpnConfig string) {
	err := json.Unmarshal([]byte(vpnConfig), &svc.vpnConfig)
	Expect(err).ToNot(HaveOccurred())
	svc.vpnConfig.PrivateKey = "private key"
	svc.vpnConfig.PublicKey = "public key"
}

func (svc *testService) Connect() (*vpn.ServiceConfig, error) {
	return &svc.vpnConfig, nil
}

func (svc *testService) Disconnect() error {
	return nil
}

func (svc *testService) GetSpaceNode() userspace.SpaceNode {
	return  svc.target
}
